...
# class average inorrect
# top 2 student incorrect
...
class Student:
    ...
    def add_score(self, score: int) -> None:
        score_f = int(score)  # store as int, not float for the 0 decimal places
        if score_f < 0 or score_f > 100:
            raise ValueError("Score out of range")
        self._scores.append(score_f) # score -> scores   
...
class Classroom:
    ...
    def add_student(self, student: Student) -> None:
        ...
        self.students[student.student_id] = student # student -> students
...
class GradeBook:
    ...
    def export_csv(self, filepath: str) -> None:
        ...
        for s in self.classroom.all_students():
            row = [s.student_id, s.name, round(s.average, 2), ";".join(str(s.scores))] # float to string
            lines.append(",".join(str(row))) # float to string
